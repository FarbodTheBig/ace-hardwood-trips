...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] flex items-center justify-center p-6">
      <div className="w-full max-w-[820px] flex rounded-2xl overflow-hidden border border-[#282828]" style={{minHeight: "500px"}}>
        {/* Left Panel */}
        <div className="flex-1 bg-[#141414] border-r border-[#282828] flex flex-col justify-center px-10 py-10">
          ...
          <div className="flex flex-col gap-3">
            {["POD & PTI photo uploads per stop", "Professional PDF trip sheets", "KM tracking & route analytics", "Direct dispatcher communication"].map((f) => (
              <div key={f} className="flex items-center gap-3">
                <div className="w-4 h-4 rounded border border-[#333] flex items-center justify-center flex-shrink-0 bg-[#1a2332]">
                  <svg className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M5 13l4 4L19 7"/></svg>
                </div>
        ...
        {/* Right Panel */}
        <div className="w-[300px] bg-[#0f0f0f] flex flex-col justify-center px-8">
          <h3 className="text-[18px] font-bold text-white mb-1">Sign in</h3>
          <p className="text-[12px] text-[#555] mb-7">Welcome back to RoadLog</p>
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            <div>
              <label className="block text-[11px] font-semibold text-[#555] uppercase tracking-widest mb-2">Email address</label>
              <input type="email" className="input-field" placeholder="you@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#555] uppercase tracking-widest mb-2">Password</label>
          ...
          <p className="text-center text-[12px] text-[#444] mt-5">
            New to RoadLog?{" "}
            <Link href="/auth/signup" className="text-[#3b82f6] hover:text-[#60a5fa]">Create account</Link>
          </p>
          <Link href="/admin/login" className="block text-center text-[11px] text-[#333] hover:text-[#555] transition-colors mt-6">
            Admin access →
          </Link>
        </div>
      </div>
