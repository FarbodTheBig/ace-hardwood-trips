...
export default function TripForm({ userId, userName }: { userId: string; userName: string }) {
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      end_date: endDate,
      start_postal_code: startPostalCode,
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            <label className={labelCls}>Start Postal Code</label>
            <input className={inputCls} value={startPostalCode} onChange={(e) => setStartPostalCode(e.target.value.toUpperCase())} placeholder="e.g. L1M 1V9" maxLength={7} />
          </div>
          <div>
            <label className={labelCls}>Destination Postal Code</label>
            <input className={inputCls} value={destinationPostalCode} onChange={(e) => setDestinationPostalCode(e.target.value.toUpperCase())} placeholder="e.g. H4R 2A2" maxLength={7} />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h2 className="section-title mb-0">
            Trips
            <span className="ml-2 bg-[#eff6ff] text-[#2563eb] text-[10px] font-bold px-2 py-0.5 rounded-full">{trips.length}</span>
          </h2>
