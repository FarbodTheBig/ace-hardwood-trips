...
export default function AdminNavbar({ adminName }: { adminName: string }) {
  ...
  return (
    <nav className="bg-gray-900 border-b border-blue-900/50 sticky top-0 z-50">
          ...
          <div className="flex items-center gap-6">
            <Link href="/admin/dashboard" className="flex items-center gap-2 shrink-0">
              ...
              <div className="hidden sm:block">
                <span className="font-bold text-white text-sm">RoadLog</span>
                <span className="text-blue-400 text-xs block font-semibold uppercase tracking-widest leading-none">Admin</span>
              </div>
            </Link>
              ...
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href}
                  className={clsx("px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap",
                    pathname.startsWith(link.href) ? "bg-blue-600/10 text-blue-400" : "text-gray-400 hover:text-white hover:bg-gray-800"
                  )}>
                  {link.label}
                </Link>
              ))}
