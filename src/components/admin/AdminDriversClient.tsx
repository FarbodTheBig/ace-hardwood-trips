    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#64748b] text-sm mt-1">{drivers.length} registered driver{drivers.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-[#d8e0ec] rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">Create Driver Account</h2>
              <button onClick={() => setShowCreate(false)} className="text-[#64748b] hover:text-white">✕</button>
            </div>
            <form onSubmit={handleCreateDriver} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#94a3b8] uppercase tracking-wider mb-1.5">Phone</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#94a3b8] uppercase tracking-wider mb-1.5">Truck #</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowCreate(false)} className="flex-1 bg-[#f8fafc] hover:bg-gray-700 text-white font-semibold px-4 py-2 rounded-lg transition-colors">Cancel</button>
      ...
      <div className="bg-white border border-[#d8e0ec] rounded-xl p-4">
        <input className="w-full bg-[#f8fafc] border border-[#d8e0ec] rounded-lg px-3 py-2 text-sm text-[#0f1a35] placeholder-[#94a3b8] focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Search by name or email..." value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>

      {filtered.length === 0 ? (
        <div className="bg-white border border-[#d8e0ec] rounded-xl p-12 text-center">
          <div className="text-4xl mb-3">👷</div>
          <p className="text-[#64748b]">No drivers found</p>
        </div>
      ) : (
        <div className="bg-white border border-[#d8e0ec] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#d8e0ec]">
                <th className="text-left py-3 px-4 text-[#64748b] font-medium">Driver</th>
                <th className="text-left py-3 px-4 text-[#64748b] font-medium hidden md:table-cell">Truck</th>
                <th className="text-right py-3 px-4 text-[#64748b] font-medium">Trips</th>
                <th className="text-right py-3 px-4 text-[#64748b] font-medium">Total KM</th>
                <th className="text-center py-3 px-4 text-[#64748b] font-medium">Status</th>
                <th className="text-right py-3 px-4 text-[#64748b] font-medium">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((driver) => (
                <tr key={driver.id} className="border-b border-[#d8e0ec]/50 hover:bg-[#f8fafc]/30">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-[#94a3b8]">{driver.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-[#0f1a35] hidden md:table-cell">{driver.truck_number || "—"}</td>
                  <td className="py-3 px-4 text-[#0f1a35] text-right">{driver.trips}</td>
                  <td className="py-3 px-4 text-red-400 font-semibold text-right">{driver.km.toLocaleString()} km</td>
                  <td className="py-3 px-4 text-center">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${driver.status === "active" ? "bg-[#f0fdf4] text-[#16a34a]" : "bg-[#f8fafc] text-[#94a3b8]"}`}>
                      {driver.status}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin/drivers/${driver.id}`} className="text-xs bg-[#f8fafc] hover:bg-gray-700 text-[#0f1a35] px-2 py-1.5 rounded-lg transition-colors">View</Link>
